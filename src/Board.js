...
export class BitBoard {
    ...
    attacksFrom(piece, index) {
        const attacks = [];
        const rank = Math.floor(index / 9), file = index % 9;
        const isBlack = piece === piece.toUpperCase();
        const colorFactor = isBlack ? -1 : 1;
        ...
        switch (piece.toUpperCase()) {
            case "P": addIfValid(colorFactor * -1, 0); break;
            case "L": slide(colorFactor * -1, 0); break;
            case "N": addIfValid(colorFactor * -2, -1); addIfValid(colorFactor * -2, 1); break;
            case "S": addIfValid(colorFactor * -1, -1); addIfValid(colorFactor * -1, 0); addIfValid(colorFactor * -1, 1); addIfValid(colorFactor * 1, -1); addIfValid(colorFactor * 1, 1); break;
            case "G":
            case "+P": case "+L": case "+N": case "+S":
                addIfValid(colorFactor * -1, -1); addIfValid(colorFactor * -1, 0); addIfValid(colorFactor * -1, 1);
                addIfValid(0, -1); addIfValid(0, 1); addIfValid(colorFactor * 1, 0);
                break;
            case "K":
    ...
    generateLegalMoves() {
                ...
                for (const to of targets) {
                    ...
                    switch (piece.toUpperCase()) {
                        case "P": case "L":
                            ...
                            else if ((isBlack && rank > 2) || (!isBlack && rank < 6)) promoteMoves = [{ promote: false }]; break;
                        case "N":
                            if ((isBlack && rank <= 1) || (!isBlack && rank >= 7)) promoteMoves = [{ promote: true }];
                            else promoteMoves = [{ promote: false }]; break;
                        case "S": case "B": case "R":
                            const inEnemyCamp = (isBlack && rank <= 2) || (!isBlack && rank >= 6);
                            promoteMoves = inEnemyCamp ? [{ promote: false }, { promote: true }] : [{ promote: false }]; break;
        ...
        for (const [piece, count] of Object.entries(this.hand[color] || {})) {
            ...
            for (let to = 0; to < 81; to++) {
                ...
                const rank = Math.floor(to / 9), file = to % 9;
                ...
                // 打てない段チェック
                if (piece.toUpperCase() === "P" || piece.toUpperCase() === "L") if ((color === "black" && rank === 0) || (color === "white" && rank === 8)) continue;
                if (piece.toUpperCase() === "N") if ((color === "black" && rank <= 1) || (color === "white" && rank >= 7)) continue;
